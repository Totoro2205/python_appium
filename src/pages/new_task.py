...
class NewTask(BasePage):
    ...
    def fill_task_input(self):
        ...
        self.driver.find_element_by_id(__task_input).send_keys("test task")

    def add_due_date(self):
        __due_date = app_package_name + "edtDueD"
        self.driver.find_element_by_id(__due_date).click()

    def click_done_button(self):
        __done_button = "android:id/button1"
        self.driver.find_element_by_id(__done_button).click()

    def add_due_time(self):
        __due_time = app_package_name + "edtDueT"
        self.driver.find_element_by_id(__due_time).click()

    def click_repeat_button(self):
        __repeat_button = app_package_name + "spinnerRepeat"
        self.driver.find_element_by_id(__repeat_button).click()

    def click_repeat_button_once_week(self):
        __repeat_button_once_a_week = text_view + "[@text='Once a Week']"
        self.driver.find_element_by_xpath(__repeat_button_once_a_week).click()

    def click_add_to_list_button(self):
        __add_to_list_button = app_package_name + "spinnerLists"
        self.driver.find_element_by_id(__add_to_list_button).click()

    def add_to_personal_list_button(self):
        __add_to_list_button_personal = text_view + "[@text='Personal']"
        self.driver.find_element_by_xpath(__add_to_list_button_personal).click()

    def save_task(self):
        __save_task = "Add Task"
        self.driver.find_element_by_accessibility_id(__save_task).click()